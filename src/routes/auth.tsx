...
function AuthPage() {
  ...
  async function google() {
    ...
    const { error } = await supabase.auth.signInWithOAuth({
      provider: "google",
      options: { redirectTo: `${window.location.origin}/grade` },
    });
    if (error) {
      setErro("Não foi possível entrar com o Google.");
    }
    // Em caso de sucesso, o Supabase redireciona a página inteira para o
    // Google e depois de volta para redirectTo — não há nada a fazer aqui.
  }
